  ...
  , mSplash( splash )
{
  ...
  mCaption = tr("QGIS - %1 ('%2')").arg(Qgis::version(), Qgis::releaseName());
  initUI(mCaption, restorePlugins, skipVersionCheck, rootProfileLocation, activeProfile);
} // QgisApp ctor
...
bool QgisApp::initUI(const QString& caption, bool restorePlugins, bool skipVersionCheck, const QString& rootProfileLocation, const QString& activeProfile)
{
  ...
  if (mActionToggleFullScreen) this->addAction(mActionToggleFullScreen);
  if (mActionTogglePanelsVisibility) this->addAction(mActionTogglePanelsVisibility);
  ...
  if (mSplash) mSplash->showMessage(tr("Reading settings"), Qt::AlignHCenter | Qt::AlignBottom);
  qApp->processEvents();

  if (mSplash) mSplash->showMessage(tr("Setting up the GUI"), Qt::AlignHCenter | Qt::AlignBottom);
  qApp->processEvents();
...
//  connect( mDevToolsDock, &QDockWidget::visibilityChanged, mActionStyleDock, &QAction::setChecked );
  ...
  // load providers
  if (mSplash) mSplash->showMessage(tr("Checking provider plugins"), Qt::AlignHCenter | Qt::AlignBottom);
  qApp->processEvents();
...
QgisApp::~QgisApp()
{
  ...
  if (mMapTools.mScaleFeature) delete mMapTools.mScaleFeature;
...
void QgisApp::createActions()
{
  ...
  if(mActionDigitizeWithCurve) connect( mActionDigitizeWithCurve, &QAction::triggered, this, &QgisApp::enableDigitizeWithCurve );
  ...
  if(mActionScaleFeature) connect( mActionScaleFeature, &QAction::triggered, this, &QgisApp::scaleFeature );
  ...
  if(mActionZoomToLayers) connect( mActionZoomToLayers, &QAction::triggered, this, &QgisApp::zoomToLayerExtent );
  ...
  if(mActionAddHanaLayer) connect( mActionAddHanaLayer, &QAction::triggered, this, [ = ] { dataSourceManager( QStringLiteral( "hana" ) ); } );
  ...
  if(mActionAddPointCloudLayer) connect( mActionAddPointCloudLayer, &QAction::triggered, this, [ = ] { dataSourceManager( QStringLiteral( "ept" ) ); } );
  ...
  if(mActionSaveEdits) menuAllEdits->addAction( mActionSaveEdits );
  if(mActionRollbackEdits) menuAllEdits->addAction( mActionRollbackEdits );
  if(mActionCancelEdits) menuAllEdits->addAction( mActionCancelEdits );
  menuAllEdits->addSeparator();
  if(mActionSaveAllEdits) menuAllEdits->addAction( mActionSaveAllEdits );
  if(mActionRollbackAllEdits) menuAllEdits->addAction( mActionRollbackAllEdits );
  if(mActionCancelAllEdits) menuAllEdits->addAction( mActionCancelAllEdits );
...
#ifdef HAVE_GEOREFERENCER
  if(mActionShowGeoreferencer) connect( mActionShowGeoreferencer, &QAction::triggered, this, &QgisApp::showGeoreferencer );
#else
  if (mActionShowGeoreferencer) delete mActionShowGeoreferencer;
...
void QgisApp::createActionGroups()
{
  ...
  if(mActionScaleFeature) mMapToolGroup->addAction( mActionScaleFeature );
  ...
  if(mActionPreviewTritanope) mActionPreviewTritanope->setActionGroup( mPreviewGroup );
}
...
void QgisApp::createMenus()
{
  ...
  if(mActionOptions) actionPrefs->setIcon( mActionOptions->icon() );
...
void QgisApp::setTheme( const QString &themeName )
{
  ...
  */
...
#endif
#ifdef HAVE_HANA
  if(mActionAddHanaLayer) mActionAddHanaLayer->setIcon( QgsApplication::getThemeIcon( QStringLiteral( "/mActionAddHanaLayer.svg" ) ) );
#endif
  ...
  if(mActionProjectProperties) mActionProjectProperties->setIcon( QgsApplication::getThemeIcon( QStringLiteral( "/mActionProjectProperties.svg" ) ) );
  ...
  if(mActionScaleFeature) mActionScaleFeature->setIcon( QgsApplication::getThemeIcon( QStringLiteral( "/mActionScaleFeature.svg" ) ) );
  ...
  if(mActionTemporalController) mActionTemporalController->setIcon( QgsApplication::getThemeIcon( QStringLiteral( "/propertyicons/temporal.svg" ) ) );
...
void QgisApp::setupConnections()
{
  ...
  if(mActionZoomLast) connect( mMapCanvas, &QgsMapCanvas::zoomLastStatusChanged, mActionZoomLast, &QAction::setEnabled );
  if(mActionZoomNext) connect( mMapCanvas, &QgsMapCanvas::zoomNextStatusChanged, mActionZoomNext, &QAction::setEnabled );
  ...
  if(mActionPreviewTritanope) connect( mActionPreviewTritanope, &QAction::triggered, this, &QgisApp::activateTritanopePreview );
...
{
  if(mLogDock) mLogDock->setUserVisible( true );
}
...
bool QgisApp::addProject( const QString &projectFile )
{
  ...
  else
  {
    ...
    if(mFilterLegendByMapContentAction) mFilterLegendByMapContentAction->setChecked( QgsProject::instance()->readBoolEntry( QStringLiteral( "Legend" ), QStringLiteral( "filterByMap" ) ) );
...
void QgisApp::updateFilterLegend()
{
  ...
  if ( mFilterLegendByMapContentAction && (mFilterLegendByMapContentAction->isChecked() || hasExpressions) )
  {
...
void QgisApp::copyLayer()
{
  ...
  // Enables the paste menu element
  if(mActionPasteLayer) mActionPasteLayer->setEnabled( true );
}
...
void QgisApp::showOptionsDialog( QWidget *parent, const QString &currentPage, int pageNumber )
{
  ...
  if ( optionsDialog->exec() )
  {
    ...
    if(mWelcomePage) mWelcomePage->updateNewsFeedVisibility();
  }
}
...
#else
  if(mActionNew3DMapCanvas) mActionNew3DMapCanvas->setVisible( false );
#endif
}
...
void QgisApp::closeProject()
{
  ...
  if(mFilterLegendByMapContentAction) mFilterLegendByMapContentAction->setChecked( false );
...
void QgisApp::activateDeactivateMultipleLayersRelatedActions()
{
  ...
  if(mActionZoomToLayers) mActionZoomToLayers->setEnabled( hasSpatial );
  if(mActionPanToSelected) mActionPanToSelected->setEnabled( hasSpatial );

  // this action is enabled whenever ANY selected layer has a selection
  const bool hasSelection = selectedLayersHaveSelection();
  if(mActionZoomToSelected) mActionZoomToSelected->setEnabled( hasSelection );
}
...
void QgisApp::activateDeactivateLayerRelatedActions( QgsMapLayer *layer )
{
  ...
  if ( !layer )
  {
    ...
    if(mActionIdentify) mActionIdentify->setEnabled( !identifyModeIsActiveLayer );
    ...
    if(mActionZoomToLayers) mActionZoomToLayers->setEnabled( false );
  ...
  if(mActionZoomToLayers) mActionZoomToLayers->setEnabled( true );
  ...
  switch ( layer->type() )
  {
    case QgsMapLayerType::VectorLayer:
    {
      ...
      if ( dprovider )
      {
        ...
        if(mActionToggleEditing) mActionToggleEditing->setEnabled( canSupportEditing );
        ...
        if(mActionScaleFeature) mActionScaleFeature->setEnabled( isEditable && canChangeGeometry );
